    ...
    <div className="w-full pt-4 px-4 sticky top-0 z-50 flex justify-center">
      <header className="w-full max-w-5xl bg-white/90 backdrop-blur-md border border-gray-200/80 shadow-sm rounded-2xl flex items-center justify-between px-6 py-2.5 relative">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Pro Plan Badge (يظهر في الشاشات الأكبر من الموبايل الصغير) */}
          ...
          {/* Login Button (زر تسجيل الدخول الجديد) */}
          <button className="px-4 py-1.5 text-sm font-medium text-white bg-purple-600 border border-gray-200 rounded-lg hover: cursor-pointer hover : transition-colors">
            Login
          </button>

          {/* Hamburger Menu Button (يظهر في الموبايل فقط) */}
        ...
        {/* Dropdown Menu for Mobile (القائمة المنسدلة للهاتف) */}
        {/* تم وضعها خارج العناصر الجانبية لضمان ظهورها بشكل سليم تحت الهيدر */}
        {isOpen && (
          <div className="absolute top-[110%] right-0 w-48 bg-white border border-gray-200 rounded-xl shadow-lg py-2 flex flex-col md:hidden">
